...
module.exports = {
  secure: {
    //ssl: true,
    ssl: false, //to run prod mode without cert set ssl to false.
    privateKey: './config/sslcerts/key.pem',
    certificate: './config/sslcerts/cert.pem'
  },
  port: process.env.PORT || 8443,
  db: {
    uri: process.env.MONGOHQ_URL || process.env.MONGOLAB_URI || 'mongodb://' + (process.env.DB_1_PORT_27017_TCP_ADDR || 'localhost') + '/training-advisor',
    options: {
      user: '',
      pass: ''
    },
  ...
  log: {
    ...
    options: {
      ...
      stream: {
        ...
        fileName: process.env.LOG_FILE || 'access-prod.log',
        rotatingLogs: { // for more info on rotating logs - https://github.com/holidayextras/file-stream-rotator#usage
          active: process.env.LOG_ROTATING_ACTIVE === 'true' ? true : false, // activate to use rotating logs
          fileName: process.env.LOG_ROTATING_FILE || 'access-prod-%DATE%.log', // if rotating logs are active, this fileName setting will be used
