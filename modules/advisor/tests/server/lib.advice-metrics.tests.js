...
//Uncaught AssertionError: expected 0 to be below 0
    it('should return dailyTargetRampRate of -0.14 if current trainingDay is last day of t6 period and no intervening workouts', function (done) {
      testHelpers.createStartingPoint(user, trainingDate, 0, 9, 9, function(err) {
        if (err) {
          console.log('createStartingPoint: ' + err);
        }

        params.numericDate = coreUtil.toNumericDate(moment(trainingDate).add(adviceConstants.minimumNumberOfTrainingDays, 'days'));

        return adviceMetrics.updateMetrics(params, function (err, trainingDay) {
          should.not.exist(err);
          should.exist(trainingDay);
          let metrics = _.find(trainingDay.metrics, ['metricsType', 'actual']);
          (trainingDay.period).should.equal('t6');
          (metrics.dailyTargetRampRate).should.equal(-0.14);
